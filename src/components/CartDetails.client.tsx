...
export function CartDetails({ onClose }: CartDetailProps) {
  const { lines } = useCart();
...
export function CartEmpty({ onClose }: CartDetailProps) {
  return (
      ...
      <div className="flex justify-between flex-1 ml-4 sm:ml-6">
        <div className="relative grid gap-1">
          ...
          <div className="flex items-center gap-2 mt-auto">
            ...
            <button
              type="button"
              onClick={() => linesRemove([lineId.toString()])}
...
function CartLineQuantityAdjust({ lineId, quantity }: { lineId: string; quantity: number }) {
  return (
    <>
